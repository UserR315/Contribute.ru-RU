...
description: Пакет расширений VS Code, упрощающий использование языка разметки Markdown для написания статей на сайте docs.microsoft.com.
author: meganbradley
ms.author: mbradley
manager: jemash
ms.date: 04/06/2018
...
- [Предварительный просмотр в DocFX] MacOS и Linux: функция предварительного просмотра DocFX неправильно запускает режим предварительного просмотра (по умолчанию включается предварительный просмотр Markdown в VS Code для этих платформ).
- [Предварительный просмотр в DocFx] Все платформы: некоторый синтаксис, например перекрестные ссылки на API, неправильно отображается в режиме предварительного просмотра, поэтому в некоторых случаях появляются пробелы в содержимом.
- [Внешние закладки] Linux: список файлов отображается, но отсутствуют заголовки для выбора.
- [Включаемые файлы] Linux: список файлов отображается, но после выбора файла ссылка не добавляется.