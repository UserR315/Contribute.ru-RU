...
Для **фрагментов кода** создайте внутри папки с вашей статьей подпапку **snippets**, если последняя еще не существует. Чаще всего вам будут нужны фрагменты кода для всех трех основных языков .NET, то есть C#, F# и Visual Basic. В этом случае для каждого из трех проектов создайте подпапки **csharp**, **fsharp** и **vb**. В руководствах конкретно по C#, F# и Visual Basic для простоты помещайте свой проект в папку **snippets**, так как в этих областях обычно нужны фрагменты кода для одного языка. Фрагменты кода — это специальные небольшие примеры кода для иллюстрации понятий, описываемых в статье. Более крупные программы, предназначенные для скачивания и самостоятельного изучения, должны находиться в репозитории [dotnet/samples](https://github.com/dotnet/samples). Полные примеры описаны в разделе [Участие в создании примеров](#contributing-to-samples).
...
### <a name="writing-a-new-sample"></a>Написание нового примера
