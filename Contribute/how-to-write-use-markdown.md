...
Так как документация хранится в репозитории GitHub, для нее можно использовать расширенный набор Markdown под названием [GitHub Flavored Markdown (GFM)](https://help.github.com/categories/writing-on-github/). Этот формат предусматривает дополнительные возможности для решения распространенных задач. Кроме того, службы Open Publishing Services (OPS) поддерживают формат DocFX Flavored Markdown (DFM). DFM обеспечивает высокий уровень совместимости с форматом GitHub Flavored Markdown (GFM), предоставляя специальные функции для работы с документацией.
...
- Раздел о [функции переноса таблиц](#table-wrapping) DFM, которая поможет при форматировании широких таблиц.
...
- Страница [Links](how-to-write-links.md) (Ссылки) этого руководства, на которой рассматриваются дополнительные возможности синтаксиса ссылок, предоставляемые DFM.
...
> [!NOTE]
> Службы Open Publishing Services (OPS) поддерживают формат DocFX Flavored Markdown (DFM), который обеспечивает высокий уровень совместимости с форматом GitHub Flavored Markdown (GFM). DFM предоставляет дополнительные функции при использовании расширений Markdown. Поэтому для справки мы включили в этот материал некоторые разделы из руководства по разработке OPS. (Например, см. разделы с инструкциями по использованию фрагментов кода и расширений DFM и Markdown.)

В статьях документации для общего форматирования, например для создания абзацев, ссылок, списков и заголовков, используется GFM. Для расширенного форматирования можно использовать функции DFM, например:
...
Чтобы ознакомиться с полным списком, см. разделы с инструкциями по использованию фрагментов кода и расширений DFM и Markdown.
...
Чтобы включить в файлы статьи многократно используемые текстовые файлы или файлы изображения, можно добавить на них ссылки с помощью функции DFM для включаемых файлов. Функция указывает OPS включить файл в файл статьи во время сборки, чтобы он стал частью опубликованной статьи. Существует три типа включаемых файлов для многократного использования содержимого:
...
- Включаемые файлы не отображаются в представлении статьи на сайте GitHub. Они используют расширения DFM Они отображаются только после публикации.
...
DFM поддерживает включение дополнительного кода в статью с помощью расширения для фрагментов кода. Этот формат обеспечивает расширенный рендеринг на основе функций GFM, таких как выбор языка программирования и раскраска синтаксиса, а также другие полезные функции, например:
